...
class FileAES:
    ...
    def ui(self):
        ...
        self.encrypt_string.set("skldfjlkskjfslkj")  # 设置加密字符串初始值
        ...
        if len(self.file_names) > 0:
            ...
            for file_path in self.file_names:
                ...
                if os.path.splitext(file_path)[1] == '.zip':
                    ...
                    self.zip_file_list = os.path.join(
                        os.path.split(file_path)[0], self.zip_file_name)
                    # folder.append(os.path.split(file_path)[1] + self.compressPostfix)
                    folder += all_file
                    self.zip_file_list += folder
                    self.zip_dir_list = []
            if handler_type == 'pack':
                self.my_zip_function(self.zip_file_name, self.zip_file_list,
                                     self.zip_dir_list)
